...
/*global jasmine: false, spyOn: false, it: false, console: false, describe: false, expect: false, beforeEach: false, waits: false, waitsFor: false, runs: false, raise: false */
/*global inject: false */
/*global MatteCtrl: false */
...
//BOARD GENERATING
    ...
    it('should pick different cells', function () {
        ...
        var cells = [];
        scope.generateBoard();
        for (x = 0, l = scope.boardSize; x < l; x++) {
            cells.push(scope.getQuestion());
        }
        for (x = 1, l = scope.boardSize; x < l; x++) {
    ...
    it('should check if someone won', function (done) {
        ...
        scope.board[0][0].player = 1;
        scope.board[0][1].player = 1;
        scope.board[0][2].player = 1;
        scope.board[0][3].player = 1;
        expect(scope.checkWin()).toBe(true);

        scope.board[0][0].player = 1;
        scope.board[0][1].player = 1;
        scope.board[0][2].player = 1;
        scope.board[0][3].player = null;
        scope.board[0][4].player = 1;
        expect(scope.checkWin()).toBe(false);
